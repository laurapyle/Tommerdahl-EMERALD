...
# changes in renal markers - adjust for sex, insulin sensitivity, percent fat 
summary_tab_adj <- NULL
BLadj2 <- function(BLVar,FLVar,label){
  a <- lm(data=alldata, FLVar ~ Randomization + BLVar + Sex + BL.Fat_per + BL.Glucose.corrected.M.I.Lean.80 )
  amod <- mod_tab(a)
  amod <- amod[amod$Predictor=="Randomization",]
  amod$Outcome <- label
  amod <- as.data.frame(amod)
...
BLadj3 <- function(BLVar,FLVar,label){
  a <- lm(data=alldata, FLVar ~ Randomization + BLVar + Sex + BL.Fat_per + BL.Glucose.corrected.M.I.Lean.80 + age)
  amod <- mod_tab(a)
  amod <- amod[amod$Predictor=="Randomization",]
  amod$Outcome <- label
  amod <- as.data.frame(amod)
...
Table 5 shows comparisons of follow-up outcomes adjusted for the baseline value of the outcome, plus sex, percent fat, and glucose corrected M/I Lean 80.

```{r, results='asis',tidy=TRUE, echo=FALSE}
kable(summary_tab_adj,caption="Table 5.  Group differences (metformin-placebo) in follow-up outcomes adjusted for baseline values, sex, percent fat, and glucose corrected M/I Lean 80.")
```
\

Table 6 shows comparisons of follow-up outcomes adjusted for the baseline value of the outcome, plus sex, percent fat, glucose corrected M/I Lean 80, and age at baseline.

```{r, results='asis',tidy=TRUE, echo=FALSE}
kable(summary_tab_adj2,caption="Table 6.  Group differences (metformin-placebo) in follow-up outcomes adjusted for baseline values, sex, percent fat, glucose corrected M/I Lean 80, and age.")
```
\